            ...
            success: function(json) {
                switch(topic) {
                    case "contact": 
                        ...
                        break;
                    case "music":
                        json.music.forEach((box) => document.getElementById("blogspace").appendChild(new Contentbox(box.cap, "", box.tracks.map((track) => `<div class="videolist_element"><p class="date">${track.name}</p><iframe style="fullsize_video" src="https://www.youtube.com/embed/${track.id}?rel=0&iv_load_policy=3" title="${track.name}" allowfullscreen=false></div>`).join(""), "").generateElement()))
                }
            },
            error:   function() {
                console.log("BRUH")
